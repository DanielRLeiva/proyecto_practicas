...
<body>
    <div class="container mt-5">
        ...
        <form action="{{ route('usufructos.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="profesor_id" id="profesor_id" class="form-select" required>
                    ...
                    @foreach ($profesores as $profesor)
                        <option value="{{ $profesor->id }}">{{ $profesor->nombre }} {{ $profesor->apellido_1 }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <button type="submit" class="btn btn-primary">Crear Usufructo</button>
        </form>

        <a href="{{ route('aulas.index') }}" class="btn btn-secondary mt-3">Volver a la lista</a>
    </div>
